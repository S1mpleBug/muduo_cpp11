...
void EventLoop::loop()
{
    ...
    while(!quit_)
    {
        ...
        /**
         * 对于线程数>=2的情况：
         * IO线程 mainLoop(mainReactor)主要工作：accept接收连接 => 将accept返回的fd打包为 => Channel
         * 因为mainLoop只做连接操作 已连接用户的打包好的Channel得分发给subLoop(subReactor)
         * 
         * mainLoop事先注册一个回调cb（该回调需要subLoop执行，但是subLoop还在poller_->poll处阻塞） mainLoop通过wakeup将subLoop唤醒
         **/
        doPendingFunctors();
    }
 ...
 * ！！！！！！！！！！！！！！！       但是 muduo 通过wakeup() 采用eventfd创建的wakeupFd_ notify 使得mainLoop和subLoop之间能够进行通信
 **/
void EventLoop::quit()
{
    quit_ = true;
...
void EventLoop::queueInLoop(Functor cb)
{
     ...
     * 需要通过wakeup写事件，让loop()下一次poller_->poll()不再阻塞（阻塞的话会影响我前一次新加入的回调的执行），然后转而继续执行回调
     **/
